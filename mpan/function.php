...
function chekb_mapCategory($category_id,$db_link){
    ...
    foreach ($cat_menyus as $line) {
        ...
        if (in_array($id, $cids))
            print "<input class='form-check-input form-check-input-primary' name='mapCategory[]' type='checkbox' value='$id' checked>";
        else
            print "<input class='form-check-input form-check-input-primary' name='mapCategory[]' type='checkbox' value='$id'>";
        print "<label class='form-check-label' for='option-1'>$ad</label></div>";   
    }
}


function combo_yemek($hefte,$category_id,$db_link){
    $cat_menyus = $db_link->get('yemekler');
    print "<select style='width: 100% !important;' name='metbex".$hefte."[]' id='metbex".$hefte."' class='form-control select2".$hefte."' multiple='multiple'>";
...
function combo_tedbir($category_id,$db_link){
    $cat_menyus = $db_link->get('tedbir_type');
    print "<select name='tid' id='tid' class='form-control'>";
    print "<option value='0'> - - - - - - - - </option>";
    foreach ($cat_menyus as $line) {
        $id = $line["id"];
        $ad = stripslashes($line["title_az"]);
...
function combo_author($category_id,$db_link){
    $cat_menyus = $db_link->get('author');
    print "<select name='author' id='author' class='form-control'>";
    print "<option value='0'> - - - - - - - - </option>";
    foreach ($cat_menyus as $line) {
        $id = $line["id"];
        $ad = stripslashes($line["title_az"]);
...
function combo_shop($category_id,$db_link){
    if($category_id==139) $stype=2;
    if($category_id==135) $stype=4;
    if($category_id==154) $stype=1;
    $cat_menyus = $db_link->where('stype',$stype)->get('shop_type');
    print "<select name='tid' id='tid' class='form-control'>";
    print "<option value='0'> - - - - - - - - </option>";
    foreach ($cat_menyus as $line) {
        $id = $line["id"];
...
function is_image($imageFileType){
